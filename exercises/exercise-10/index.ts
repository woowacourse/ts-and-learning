...
export function promisifyAll<T extends Object>(oldApi: T): { [K in keyof T]: () => Promise<any> } {
  const apiEntries = Object.entries(oldApi);
  const apiMap = apiEntries.map((requestFunction) => {
    return [requestFunction[0], promisify(requestFunction[1])];
  });
  const apiObject = Object.fromEntries(apiMap);
  return apiObject;
}
