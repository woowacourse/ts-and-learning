...
type CallbackFunction<T> = (response: ApiResponse<T>) => void;
type RequestFunction<T> = (callback: CallbackFunction<T>) => void;

export function promisify<T>(arg: RequestFunction<T>): () => Promise<T> {
  return () =>
...
type Promisified<T> = {
  [K in keyof T]: T[K] extends RequestFunction<infer R> ? () => Promise<R> : never;
};

function promisifyAll<T extends { [key: string]: any }>(obj: T): Promisified<T> {
  const newObj: any = {};
