...
const test1 = new ObjectManipulator({}).set('x', 123).set('y', 'hello').getObject();
...
const test3 = new ObjectManipulator({}).set('x', 123).set('y', 'hello').delete('y').get('x');

typeAssert<IsTypeEqual<typeof test3, number>>();

const test4 = new ObjectManipulator({x: true, y: 'hello'}).delete('y').get('x');
